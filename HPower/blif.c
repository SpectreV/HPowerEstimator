...
static boolean add_activity_to_net(char * net_name, float probability,
		float density);
...
static void read_activity(char * activity_file) {
	...
	printf("Reading activity file %s...............................\n",
			activity_file);
	...
	blif_net_activity = (t_net_activity *) my_malloc(
			num_logical_nets * sizeof(t_net_activity));

	for (net_idx = 0; net_idx < num_logical_nets; net_idx++) {
		blif_net_activity[net_idx].probability = -1.0;
		blif_net_activity[net_idx].density = -1.0;
	}
	...
	for (net_idx = 0; net_idx < num_logical_nets; net_idx++) {
		if (blif_net_activity[net_idx].probability < 0.0
				|| blif_net_activity[net_idx].density < 0.0) {
			printf("Error: Activity file does not contain signal %s\n",
					vpack_net[net_idx].name);
			fail = TRUE;
		}
		/*printf("VPACK_NET#%d, prob = %lf, sa = %lf\n", net_idx,
				blif_net_activity[net_idx].probability,
				blif_net_activity[net_idx].density);*/
	}
...
boolean add_activity_to_net(char *net_name, float probability, float density) {
	...
	while (h_ptr != NULL) {
		if (strcmp(h_ptr->name, net_name) == 0) {
			net_idx = h_ptr->index;
			blif_net_activity[net_idx].probability = probability;
			blif_net_activity[net_idx].density = density;
			return FALSE;
		}
		h_ptr = h_ptr->next;
	}
