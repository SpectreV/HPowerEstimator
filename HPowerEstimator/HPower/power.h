...
typedef struct s_net_activity {
	...
	/* Transition density - average # of transitions per clock cycle
...
typedef struct s_power_instance {
	int index;
	char *name;
	int depth;
	float at_freq;
	float dynamic_power_avg;
	int input_size;
	int output_size;
	int num_conf_bits;
	float static_power_avg;
	/*different static power numbers depend on different input vectors and output vectors*/
	t_power_type type;
	int num_children_types;
	struct s_power_instance *children, *parent;
}t_power_instance;
...
void ProcessPbPower(ezxml_t Node, t_power_instance *instance, t_power_instance *parent);
