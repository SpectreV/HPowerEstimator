...
float pb_max_internal_delay = 0;  /* biggest internal delay of physical block */
     ...
     char **argv)
{
    ...
    SetupVPR(Options, TimingEnabled, &FileNameOpts, &Arch, &Operation, &user_models, 
	     ...
	     &AnnealSched, &RouterOpts, &PowerOpts, &RoutingArch, &Segments,
	...
	if(PackerOpts.doPacking) {
		begin = clock();
		try_pack(&PackerOpts, &Arch, user_models, library_models, PowerOpts.do_power, Options.ActivityFile);
		end = clock();
	...
	/*power_estimation(PowerOpts.power_library);*/
