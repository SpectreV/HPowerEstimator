...
static int get_bidir_track_to_chan_seg(INP struct s_ivec conn_tracks,
		INP t_ivec *** rr_node_indices, INP int to_chan, INP int to_seg,
		INP int to_sb, INP t_rr_type to_type, INP t_seg_details * seg_details,
		INP boolean from_is_sbox, INP int from_switch,
		INOUTP boolean * rr_edge_done, INP enum e_directionality directionality,
...
static int get_unidir_track_to_chan_seg(INP boolean is_end_sb,
		...
		INP t_rr_type to_type, INP int nodes_per_chan, INP int nx, INP int ny,
		INP enum e_side from_side, INP enum e_side to_side, INP int Fs_per_side,
		INP int *opin_mux_size, INP short *****sblock_pattern,
		INP t_ivec *** rr_node_indices, INP t_seg_details * seg_details,
		INOUTP boolean * rr_edge_done, OUTP boolean * Fs_clipped,
		INOUTP struct s_linked_edge **edge_list);
...
static int *label_wire_muxes_for_balance(INP int chan_num, INP int seg_num,
		...
		INP int *opin_mux_size, INP t_ivec *** rr_node_indices);
	...
	*nodes_per_chan = tmp;
	...
	for (i = 0; i < num_seg_types; ++i) {
		...
		for (itrack = 0; itrack < ntracks; itrack++) {
			...
			seg_details[cur_track].group_size = min(
					ntracks + first_track - group_start, length * fac);
...
int get_bidir_opin_connections(INP int i, INP int j, INP int ipin,
		...
		INP int Fc, INP boolean * rr_edge_done, INP t_ivec *** rr_node_indices,
		INP t_seg_details * seg_details) {
	...
	for (iside = 0; iside < 4; iside++) {
		...
		for (iconn = 0; iconn < Fc; ++iconn) {
			...
			if (OPEN == to_track || is_connected_track) {
				...
				assert(
						OPEN
								== opin_to_track_map[type->index][ipin][ofs][iside][0]);
				continue;
			}
...
int get_unidir_opin_connections(INP int chan, INP int seg, INP int Fc,
		...
		INOUTP boolean * rr_edge_done, INP int max_len, INP int nodes_per_chan,
		INP t_ivec *** rr_node_indices, OUTP boolean * Fc_clipped) {
...
struct s_ivec ***
alloc_and_load_rr_node_indices(INP int nodes_per_chan, INP int nx, INP int ny,
	...
	for (i = 0; i <= (nx + 1); ++i) {
		indices[IPIN][i] = (t_ivec *) my_malloc(sizeof(t_ivec) * (ny + 2));
	...
	load_chan_rr_indices(nodes_per_chan, nx + 1, ny + 1, CHANX, seg_details,
			index, indices);
	load_chan_rr_indices(nodes_per_chan, ny + 1, nx + 1, CHANY, seg_details,
			index, indices);
...
int get_track_to_tracks(INP int from_chan, INP int from_seg, INP int from_track,
		...
		INP t_seg_details * seg_details,
		INP enum e_directionality directionality,
		INP t_ivec *** rr_node_indices, INOUTP boolean * rr_edge_done,
	...
	assert(
			from_seg
					== get_seg_start(seg_details, from_track, from_chan,
							from_seg));
		...
		if (from_sb < from_end) {
			if (BI_DIRECTIONAL == directionality) {
				...
				num_conn += get_bidir_track_to_chan_seg(conn_tracks,
						rr_node_indices, to_chan, to_seg, to_sb, to_type,
						seg_details, from_is_sbox, from_switch, rr_edge_done,
			...
			if (UNI_DIRECTIONAL == directionality) {
				...
				if ((from_is_sbox)
					...
					num_conn += get_unidir_track_to_chan_seg(
							...
							sblock_pattern, rr_node_indices, seg_details,
							rr_edge_done, &Fs_clipped, edge_list);
				}
			}
		}
			...
			if (UNI_DIRECTIONAL == directionality) {
				...
				if ((from_is_sbox)
					...
					num_conn += get_unidir_track_to_chan_seg(
							...
							rr_node_indices, seg_details, rr_edge_done,
...
short *****
alloc_sblock_pattern_lookup(INP int nx, INP int ny, INP int nodes_per_chan) {
