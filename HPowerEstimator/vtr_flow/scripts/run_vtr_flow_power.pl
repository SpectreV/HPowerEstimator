...
{
	print("usage: run_vtr_flow.pl <architecture_file> <circuit_file> <power_library_file> [OPTIONS]\n");
	exit(-1);
}
...
(-f $power_lib_file_path) or die "Power library file not found ($power_lib_file_path)";

# Select executables
my $hpower_path = "$vtr_flow_path/../HPower/hpower";
(-e $hpower_path) or die "Cannot find hpower executable ($hpower_path)";

my $vpr_path = "$vtr_flow_path/../vpr/vpr";
(-e $vpr_path) or die "Cannot find vpr exectuable ($vpr_path)";

my $ace_path = "$vtr_flow_path/../ace2/ace";
(-e $ace_path) or die "Cannot find ace executable ($ace_path)";
...
$power_lib_file_path =~ m/.*\/(.*?.xml)/;
my $power_lib_file_name = $1;

$power_lib_file_name =~ m/(.*).xml$/;
my $power_lib_name = $1;
print "$architecture_name/$benchmark_name/$power_lib_name...\n";
...
my $ace_output_blif_file_name = "$benchmark_name" . ".blif";
my $ace_output_blif_file_path = "$temp_dir$ace_output_blif_file_name";

my $ace_output_act_file_name = "$benchmark_name" . ".act";
my $ace_output_act_file_path = "$temp_dir$ace_output_act_file_name";
...
my $vpr_netlist_output_file_name = "$benchmark_name" . ".net";
my $vpr_netlist_output_file_path = "$temp_dir$vpr_netlist_output_file_name";

my $vpr_place_output_file_name = "$benchmark_name" . ".place";
my $vpr_place_output_file_path = "$temp_dir$vpr_place_output_file_name";
...
my $power_lib_file_path_new = "$temp_dir$power_lib_file_name";
copy ($power_lib_file_path, $power_lib_file_path_new);
$power_lib_file_path = $power_lib_file_path_new;
...
if ($ending_stage >= $stage_idx_ace and $starting_stage <= $stage_idx_ace and ! $error_code)
{
	$q = &system_with_timeout($ace_path,
									"ace.out",
									$timeout,
									$temp_dir,
									"-b", "$scripts_output_file_name",
									"-n", "$ace_output_blif_file_name",
									"-o", "$ace_output_act_file_name",
									"-d", 0.5
	);
	(-e $ace_output_act_file_path) or die "ace-2 failed";
	print "ACE-2: successful\n";
}
...
if ($ending_stage >= $stage_idx_vpr and $starting_stage <= $stage_idx_vpr and ! $error_code)
{
	if($min_chan_width < 0) {
		$q = &system_with_timeout($vpr_path, 
									...
									"$benchmark_name",
									"--blif_file", "$ace_output_blif_file_name",									
...
if ($ending_stage >= $stage_idx_hpower)
{
	$q = &system_with_timeout($hpower_path,
									"$benchmark_name" . ".power",
									$timeout,
									$temp_dir,
									$architecture_file_name,
									"$benchmark_name",
...
sub stage_index
{
	...
	if (lc($stage_name) eq "hpower")
	{
		return $stage_idx_hpower;
	}
	return -1;	
}
